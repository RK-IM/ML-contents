...
class XrayClassifyAdaptor(XrayDatasetAdaptor):
    def __init__(self, images_dir, annotations_df, meta_df):
        ...
        self.annotations_df = self.annotations_df.drop_duplicates("image_id").reset_index(drop=True)
    
    def get_image_and_labels_by_idx(self, index):
        image_name = self.images[index]
        image = Image.open(os.path.join(self.images_dir, image_name) + ".png")
        label = self.annotations_df[self.annotations_df["image_id"]==image_name]["class_id"].values[0]
...
class XrayDetectAdaptor(XrayDatasetAdaptor):
    def __init__(self, images_dir, annotations_df, meta_df, n_splits=5, fold_index=4, train=False):
        super().__init__(images_dir, annotations_df, meta_df)

        self.n_splits = n_splits
        self.fold_index = fold_index

        self.annotations_df = self.annotations_df.query("class_id != 14").reset_index(drop=True)
    ...
    def get_image_and_labels_by_idx(self, index):
        ...
        size = self.meta_df.query("image_id==@image_name")[["height", "width"]].values[0]
        return image, pascal_bboxes, class_labels, image_name, size
    
    def show_image(self, index):
        image, pascal_bboxes, class_labels, image_name, size = self.get_image_and_labels_by_idx(index)
    ...
    def add_sgkf_fold(self, n_splits=5):
        ...
        for i, (_, fold_idx) in enumerate(sgkf.split(self.annotations_df,
...
class XrayDetectDataset(Dataset):
    def __init__(self, dataset_adaptor, transforms=valid_transforms_det(), bboxes_yxyx=True):
        super().__init__()
        self.adaptor = dataset_adaptor
        self.transforms = transforms
    ...
    def __getitem__(self, index):
        image, pascal_bboxes, class_labels, image_name, size = \
        ...
        sample = {
            ...
            "bboxes": pascal_bboxes / [size[1], size[0], size[1], size[0]] * image.shape[0], 
            "labels": class_labels,
        }
